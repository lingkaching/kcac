...
class BSSEnv(gym.Env):
    ...
    def __init__(self, nzones=95, ntimesteps=12, data_dir=None, data_set_name='actual_data_art', constraints=None, scenarios=list(range(1, 21))):
        super().__init__()
        self.nzones = nzones
        ...
        if constraints is not None:
            self.__cp = self.__retrieve_capacities_from_constraint_dict(self.constraints, constraints['equals'] == 1)
        assert len(self.__cp) == nzones, "constraint is ill-defined"
    ...
    def __retrieve_capacities_from_constraint_dict(self, node, normalized):
        capacity = {}
        if 'children' not in node.keys():
            if node['zone_id'] not in capacity.keys():
                capacity[node['zone_id']] = node['max']
        elif len(node['children']) == 0:
            if node['zone_id'] not in capacity.keys():
                capacity[node['zone_id']] = node['max']
        else:
            for child in node['children']:
                capacity.update(self.__retrieve_capacities_from_constraint_dict(child, normalized))
        if node ['name'] == 'root':
            capacities = [0] * len(capacity)
            for key, value in capacity.items():
                if normalized:
                    value = value * self.nbikes
    ...
    def __read_capacity_and_starting_allocation(self, filename):
        # capacities are read from file but not used anymore! they are set via the constraint argument provided!
